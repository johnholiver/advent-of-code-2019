...
func Test_part1(t *testing.T) {
	m := computer_mem.NewMemory("1,0,0,0,99")
	p := computer.NewProcessor(nil, nil, m)
	p.Process()
	assert.Equal(t, "2,0,0,0,99", m.String())

	m = computer_mem.NewMemory("2,3,0,3,99")
	p = computer.NewProcessor(nil, nil, m)
	p.Process()
	assert.Equal(t, "2,3,0,6,99", m.String())

	m = computer_mem.NewMemory("2,4,4,5,99,0")
	p = computer.NewProcessor(nil, nil, m)
	p.Process()
	assert.Equal(t, "2,4,4,5,99,9801", m.String())

	m = computer_mem.NewMemory("1,1,1,4,99,5,6,0,99")
	p = computer.NewProcessor(nil, nil, m)
	p.Process()
